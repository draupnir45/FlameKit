...
class FlameScrollView: UIScrollView, UITextFieldDelegate {
...
//  var constraintForBouncing: NSLayoutConstraint?
  
  var shouldAddNavBarInset: Bool = false {
    didSet {
...
//  var shouldBounceWhenContentDidNotReachBottom: Bool = true {
//    willSet {
//      if let constraintForBouncing = constraintForBouncing {
//        constraintForBouncing.isActive = newValue
//      }
//    }
//  }
...
//    let constraintForBouncing = 
//      NSLayoutConstraint(item: contentView, attribute: .height, relatedBy: .greaterThanOrEqual, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: self.bounds.height)
//    constraintForBouncing.priority = UILayoutPriority(rawValue: 800)
    
//    self.constraintForBouncing = constraintForBouncing
    let constraints = [
      // contentView
...
//      wself.constraintForBouncing?.isActive = false
    ...
    keyboardWillHideObserver = NotificationCenter.default.addObserver(forName: NSNotification.Name.UIKeyboardWillHide, object: nil, queue: nil) {[weak self] _ in
      ...
      wself.contentInset = calculatedContentInsets
      wself.scrollIndicatorInsets = calculatedScrollIndicatorInsets
//      wself.constraintForBouncing?.isActive = true
      wself.setNeedsLayout()
      wself.layoutIfNeeded()
    }
  }
...
//    constraintForBouncing?.constant = self.bounds.height + 1.0 - self.contentInset.top
    layoutIfNeeded()
  }
  ...
  func addArrangedSubview(_ view: UIView, animated: Bool = false, horizontalInset: CGFloat = 0.0, height: CGFloat = 0.0) {
  ...
  ///   - height: 마진의 높이입니다.
  ///   - color: 마진 색입니다.
  func addMarginStack(height: CGFloat, color: UIColor = .clear) {
